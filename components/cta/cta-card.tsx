...
export function CtaCard({
  ...
  const isDarkSurface = isImage || variant === "solid";
  return (
    <div
      className={cn(
      ...
      <div className="relative z-20 max-w-2xl">
        ...
        {primary || secondary ? (
          <div className="mt-7 flex flex-wrap gap-3">
            {primary ? (
              <CtaButton href={primary.href} variant={isDarkSurface ? "light" : "primary"}>
                {primary.label}
              </CtaButton>
            ) : null}
            {secondary ? (
              <CtaButton href={secondary.href} variant={isDarkSurface ? "outlineLight" : "outline"}>
                {secondary.label}
              </CtaButton>
            ) : null}
          </div>
