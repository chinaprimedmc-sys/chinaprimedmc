...
import { popularSearches, searchDiscovery } from "@/content/discovery";
...
export function SearchResultsTemplate({
  ...
  const results = searchDiscovery(discoveryIndex, query, filters);
      ...
      <Section spacing="default">
        <ContentContainer size="xl" className="grid gap-8">
          <SectionHeader
            ...
            description="A few strong next steps based on what travelers usually compare at this stage."
          />
      ...
      <Section spacing="default" className="bg-white">
        <ContentContainer size="xl">
          <CtaCard
            ...
            primary={{ label: "Ask for Route Ideas", href: planJourneyHref }}
            secondary={{
...
function SearchHero({ query }: { query: string }) {
  return (
    <section className="relative isolate grid min-h-[76svh] overflow-hidden">
      <OptimizedImage
        src={destinationAsset.huangshanClouds.src}
        alt={destinationAsset.huangshanClouds.alt}
        fill
        priority
        sizes="100vw"
        objectPosition={destinationAsset.huangshanClouds.objectPosition}
      ...
      <ContentContainer
        size="xl"
        className="relative z-30 flex flex-col justify-center py-28 text-white md:py-36"
      >
        <div className="max-w-5xl">
          <Badge className="bg-white/18 text-white backdrop-blur-xl">Discovery</Badge>
          ...
          <div className="mt-7 flex flex-wrap gap-2">
            {popularSearches.slice(0, 4).map((term) => (
              <a key={term} href={`/search?q=${encodeURIComponent(term)}`}>
                <Badge className="bg-white/18 text-white backdrop-blur-xl">{term}</Badge>
              </a>
            ))}
          </div>
        </div>
