...
const catalogSummary = journeyCatalog
  .map(
    (journey) =>
      `- [${journey.title}](${new URL(journey.href, siteConfig.url)}): ${journey.commercialRoleLabel}; ${journey.durationLabel}; route ${journey.routeLabel}; from US$${journey.pricing.fromUsd.toLocaleString("en-US")} ${journey.slug === "private-shanghai-day-tour-guide-driver" ? "per private group of four, equivalent to US$168 per guest" : journey.slug === "private-mutianyu-great-wall-day-tour" ? "per guest based on four guests traveling privately" : "per person based on four guests sharing two rooms"}; best for ${journey.bestForSummary}.`,
  )
  .join("\n");
